const TRANSLATIONS = {
  ...
  welcomeMessage: {
    part1:
      "Outamation AI에 오신 것을 환영합니다. Outamation AI은 Mintplex Labs에서 개발한 오픈 소스 AI 도구로, 어떤 것이든 훈련된 챗봇으로 변환하여 쿼리하고 대화할 수 있습니다. Outamation AI은 BYOK(Bring Your Own Key) 소프트웨어이므로 사용하려는 서비스 외에는 구독료나 기타 비용이 없습니다.",
    part2:
      "Outamation AI은 OpenAi, GPT-4, LangChain, PineconeDB, ChromaDB 등 강력한 AI 제품을 번거로움 없이 깔끔하게 패키지로 묶어 생산성을 100배 향상시키는 가장 쉬운 방법입니다.",
    part3:
      "Outamation AI은 로컬 컴퓨터에서 완전히 작동하며, 거의 리소스를 사용하지 않으므로 존재조차 느끼지 못할 것입니다! GPU가 필요하지 않습니다. 클라우드 및 온프레미스 설치도 가능합니다.\nAI 도구 생태계는 날로 강력해지고 있습니다. Outamation AI은 이를 쉽게 사용할 수 있게 해줍니다.",
  ...
  appearance: {
    ...
    message: {
      ...
      user: "사용자",
      message: "메시지",
      assistant: "Outamation AI 채팅 어시스턴트",
  ...
  api: {
    title: "API 키",
    description:
      "API 키는 소유자가 프로그래밍 방식으로 이 Outamation AI 인스턴스에 액세스하고 관리할 수 있도록 합니다.",
    link: "API 문서 읽기",
    generate: "새 API 키 생성",
    table: {
      key: "API 키",
  ...
  llm: {
    title: "LLM 기본 설정",
    description:
      "이것은 채팅과 임베딩을 하기 위한 선호하는 LLM 제공자의 인증입니다. 이 키가 현재 활성 상태이고 정확해야 Outamation AI이 제대로 작동합니다.",
    provider: "LLM 제공자",
  },
  ...
  transcription: {
    ...
    provider: "텍스트 변환 제공자",
    "warn-start":
      "RAM 또는 CPU 성능이 제한된 머신에서 로컬 위스퍼 모델을 사용하면 미디어 파일을 처리할 때 Outamation AI이 중단될 수 있습니다.",
  ...
  embedding: {
    ...
    "desc-end":
      "임베딩은 텍스트를 벡터로 변환하는 과정입니다. 파일과 프롬프트를 Outamation AI이 처리할 수 있는 형식으로 변환하려면 이러한 인증이 필요합니다.",
    provider: {
      title: "임베딩 제공자",
      description:
        "Outamation AI의 기본 임베딩 엔진을 사용할 때는 설정이 필요하지 않습니다.",
    },
  },
  ...
  vector: {
    title: "벡터 데이터베이스",
    description:
      "이것은 Outamation AI 인스턴스가 벡터 데이터베이스 사용을 위한 인증 설정입니다. 이 키가 활성 상태이고 정확해야 합니다.",
    provider: {
      title: "벡터 데이터베이스 제공자",
  ...
  multi: {
    ...
    password: {
      title: "비밀번호 보호",
      description:
        "Outamation AI 인스턴스를 비밀번호로 보호하십시오. 이 비밀번호를 잊어버리면 복구 방법이 없으므로 반드시 저장하세요.",
    },
    instance: {
      title: "인스턴스 비밀번호 보호",
      description:
  ...
  privacy: {
    title: "개인정보와 데이터 처리",
    description:
      "연결된 타사 제공자와 Outamation AI이 데이터를 처리하는 방식을 구성합니다.",
    llm: "LLM 선택",
    embedding: "임베딩 기본 설정",
    vector: "벡터 데이터베이스",
